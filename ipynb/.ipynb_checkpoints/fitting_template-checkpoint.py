...
class FeXII_fit:
    ...
    def plot(self,plot_fit = True,plot_mcmc = False):
        if self.shape[0] == 32:
            ...
            fig, axes = plt.subplots(nrows,4,figsize=(12,nrows*2))
        
        
        for ii, ax_ in enumerate(axes.flatten()):
            if ii < self.shape[0]:
                if self.error is None:
                    ln1 = ax_.step(self.wvl,self.data[ii,:],where="mid",color="#00274C",label = r"$I_{\rm obs}$")
                else:
                    ln1 = ax_.errorbar(self.wvl,self.data[ii,:],yerr = self.error[ii,:],ds='steps-mid',color="#F596AA",capsize=2,label = r"$I_{\rm obs}$")
                if plot_fit:
                        ...
                        if plot_mcmc:
                            ...
                            if self.stray_int != 0:
                                ...
                                if self.stray_wvl == 0:
                                    ...
                                    g_stray = myGaussian1D(intensity=self.stray_int, mean=self.stray_wvl, 
                                        fwhm = self.stray_fwhm,bg = 0,stray_int = 0, stray_fwhm = 1,stray_wvl=self.stray_wvl)                
                        ln2, = ax_.plot(self.wvl,g_fit(self.wvl),color="#FFCB05",ls="-.",label = r"$I_{\rm fit}$")
                        if self.stray_int != 0:
                            ln3, = ax_.plot(self.wvl,g_true(self.wvl),color="#90B44B",ls="-.",label = r"$I_{\rm pure}$")
                            ln4, = ax_.plot(self.wvl,g_stray(self.wvl),color="#66BAB7",ls="-.",label = r"$I_{\rm stray}$")
        ...
        if self.stray_int != 0:
            leg = [ln1, ln2, ln3, ln4]
            axes[0,0].legend(leg,[leg_.get_label() for leg_ in leg],bbox_to_anchor=(-0.03,1.1,1,0.2), loc="upper left",ncol=4)
        else:
            leg = [ln1, ln2]
            axes[0,0].legend(leg,[leg_.get_label() for leg_ in leg],bbox_to_anchor=(-0.03,1.1,1,0.2), loc="upper left",ncol=2)
        ...
        if self.fit_type == "Single":
            for ii in range(self.shape[0]):
                if ii == 0:
                    ...
                    fwhm = self.result[ii,2],bg = 0.,stray_int = self.stray_int,stray_fwhm = self.stray_fwhm,stray_wvl = self.stray_wvl)
                else:
