...
c1 = 0.1415e-3
c2 = 0.0205e-6
wvl_cor = c1*(findgen(512)+ypix_start) + c2*(findgen(512)^2+ypix_start)
n_wvl = 976 ;4096
...
window_start_index = intarr(23)
window_end_index = intarr(23)
filenames = file_search('../raw_data/south_pole/level1/eis_l1_*.fits')

index_start = 0
for ii = 0, sizeof(filenames) - 1 do begin
    print,filenames[ii]
    new_data_offset = dblarr(n_wvl,534,n_raster)
    new_err_offset = dblarr(n_wvl,534,n_raster)
    wvl = dblarr(n_wvl)
    raw_image = dblarr(n_wvl,512,n_raster)
    raw_image_err = dblarr(n_wvl,512,n_raster)
...
;        print,data->getexp()
;    endif
    save,filename = "../save/new_lvl1_offset_limb_1025_southpole_tilt_cor"+strmid(filenames[ii],23,18,/reverse_offset)+".sav",wvl,new_data_offset,new_err_offset, $
